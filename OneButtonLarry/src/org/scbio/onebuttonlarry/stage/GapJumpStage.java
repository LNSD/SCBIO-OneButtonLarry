...
public class GapJumpStage extends GameStage {
	...
	public  GapJumpStage(Context context, GameView parent) {
		this.parent = parent;
	...
	private class JumpLarry extends Larry{
		...
		{
			boolean firstgap = (getPosX() > 0.29*parent.getWidth() && getPosX() < 0.36*parent.getWidth());
			boolean secondgap = (getPosX() > 0.5*parent.getWidth() && getPosX() < 0.53*parent.getWidth()); 
			boolean result = (firstgap || secondgap) && !jump;
